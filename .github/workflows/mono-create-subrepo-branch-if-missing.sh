...
find_matching_commit_in_subrepo() {
  ...
  while [ -n "$COMMIT" ]; do
      local found_commit=$(git -C "$SUBREPO_PATH" log --all --grep="GitOrigin-RevId: $COMMIT" --format="%H" -n 1)
...
find_closest_merge_base_on_release_branches() {
  ...
  local current_branch=$(git rev-parse --abbrev-ref "$BRANCH_NAME")
  local branches=$(gh api "repos/$REPO_NAME/branches" --paginate --jq '.[].name' | grep -E '^[0-9]+\.[0-9]+\.[0-9z]+(-BETA-[0-9]+)?$|^master$|^data-migration-5.3$')
  ...
  for branch in $branches; do
    ...
    local merge_base_on_branch=$(git merge-base "$BRANCH_NAME" "origin/$branch")
