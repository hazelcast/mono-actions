...
get_ancestor_distance() {
    local commit_hash="$1"
    local path_length=$(git rev-list --ancestry-path --count "${commit_hash}..HEAD")
    echo "$path_length"
}
...
find_closer_ancestor() {
    ...
    if [ -z "$commit1" ] && [ -z "$commit2" ]; then
        ...
        local length_commit1=$(get_ancestor_distance $commit1)
        local length_commit2=$(get_ancestor_distance $commit2)
