...
function test_get_master_version() {
  ...
  MOCK_POM_VERSION="5.5.0"
  actual=$(get_master_version "${TEST_REPO}")
  msg="Correctly retrieves master pom file version"
  assert_eq "5.5.0" "${actual}" "${msg}" && log_success "${msg}" || TESTS_RESULT=$?
